...
public class ServiceOnOffJob extends JobAdapter {
	...
	private void statefulSetOnOff(String namespace, String stsName, int toggle) {
...
//			statefulSets.createOrReplace(newSts);
	...
	private void deploymentOnOff(String namespace, String stsName, int toggle) {
		try (DefaultKubernetesClient kubernetesClient = K8SUtil.kubernetesClient();){
	...
	public void run() {
		...
		if("mariadb".equals(param.getServiceType())) {
			statefulSetOnOff(namespace, stsName, toggle);
		} else if("redis".equals(param.getServiceType())) {
			if(stsName.endsWith("-master")) {
				statefulSetOnOff(namespace, stsName, toggle);
			} else if(stsName.endsWith("-slave")) {
				deploymentOnOff(namespace, stsName, toggle);
			}
		}
