...
public interface RestoreEntityRepository extends CrudRepository<RestoreEntity, String> {
    ...
    @Query(value = "select * from restore_entity where target_backup_id = :targetBackupId and service_name = :serviceName and status = 'ACCEPTED' order by accepted_datetime desc limit 1", nativeQuery = true)
