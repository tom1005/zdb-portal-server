...
//	List<DiskUsage> findGroupByReleaseName();
	...
	@Query(value = "select pod_name, namespace, release_name, size, avail, used, use_rate, path, update_time from zdb.disk_usage where pod_name=:pod_name", nativeQuery = true)
	List<DiskUsage>  findByPodName(@Param("pod_name") String pod_name);
